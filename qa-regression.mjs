...
check("Publicação em 8 etapas", /publishBody\(step\)/.test(app) && /step<8/.test(app));
check("Pagamento protegido", /appPayment/.test(app) && /setupPaymentFlow/.test(app));
check("Rastreamento de entrega", /appDelivery/.test(app) && /setupDeliveryTracking/.test(app));
check("Painel administrativo", ["admin/dashboard", "admin/operations", "admin/reports"].every((route) => routes.has(route)));
check(
...
check("Publicação web em 8 etapas", routes.has("site/publish") && /function setupSitePublish/.test(app) && /data-site-publish-step/.test(app) && /Splits aprovados/.test(app) && /Adicione pelo menos uma foto/.test(app) && /Anúncio publicado/.test(app));
...
check("Reserva com regras, cancelamento e expiração", ["app/reservation-rules","app/reservation-cancel","app/reservation-cancelled","app/reservation-expired"].every((route)=>routes.has(route)) && /function appReservationRules/.test(app) && /function appReservationCancel/.test(app) && /reservation-cancel-form/.test(app) && /Cancelar reserva/.test(app) && /Reserva cancelada/.test(app) && /Reserva expirada/.test(app) && /location.hash='#\/app\/reservation-expired'/.test(app));
