...
function resolveRequest(pathname) {
  ...
  if (surface === "admin" && ["/admin/users", "/admin/roles", "/admin/ads", "/admin/operations", "/admin/finance", "/admin/logistics", "/admin/reports", "/admin/data", "/admin/forecast", "/admin/alerts", "/admin/infrastructure", "/admin/security", "/admin/moderation", "/admin/audit", "/admin/settings"].includes(pathname)) {
    return resolve(appRoot, `${pathname.split("/").at(-1)}.html`);
  }
  if (surface === "app" && ["/app/onboarding", "/app/login", "/app/register", "/app/recovery", "/app/verify", "/app/home", "/app/search", "/app/favorites", "/app/chat", "/app/safety", "/app/proposal", "/app/proposal-received", "/app/negotiation-evidence", "/app/reservation", "/app/reservation-rules", "/app/reservation-cancel", "/app/reservation-cancelled", "/app/reservation-expired", "/app/payment", "/app/payment-success", "/app/wallet", "/app/wallet-cards", "/app/refund", "/app/dispute", "/app/delivery-options", "/app/delivery", "/app/delivery-reschedule", "/app/delivery-proof", "/app/delivery-confirm", "/app/delivery-rate", "/app/auth-error", "/app/auth-locked", "/app/auth-offline", "/app/notifications", "/app/notification-settings", "/app/profile", "/app/profile-ads", "/app/profile-history", "/app/profile-reviews", "/app/addresses", "/app/settings", "/app/my-impact", "/app/help", "/app/state-offline", "/app/state-permission", "/app/state-removed"].includes(pathname)) {
    return resolve(appRoot, `${pathname.split("/").at(-1)}.html`);
  }
